...
CREATE OR REPLACE FUNCTION "public"."get_student_assignments_unified"("target_user_id" "uuid") RETURNS TABLE("assignment_id" "uuid", "assignment_type" "text", "title" "text", "assigned_at" timestamp with time zone, "assigned_by_username" "text", "due_date" timestamp with time zone, "completed" boolean, "completed_at" timestamp with time zone, "is_overdue" boolean, "content_data" "jsonb", "assignment_level" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
DECLARE

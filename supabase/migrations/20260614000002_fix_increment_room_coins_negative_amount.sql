-- Migration: Fix daily earned amount logic in increment_room_coins for negative amounts (reverts)
-- Description: Updates increment_room_coins so that negative coin adjustments (like morning duty overrides)
--              correctly adjust the student's daily earned coin count (daily_real_amount).
...
BEGIN
    ...
    -- Logic for incrementing daily counts (Fixed to support negative reverts)
    IF NOT p_skip_daily_count THEN
        IF amount > 0 THEN
            ...
            END IF;
        ELSIF amount < 0 THEN
            -- Deduct from daily real earned amount
            daily_real_amount := GREATEST(0, daily_real_amount + amount);
            IF is_answering_question THEN
                daily_real_count := GREATEST(0, daily_real_count - 1);
            END IF;
        END IF;
    END IF;

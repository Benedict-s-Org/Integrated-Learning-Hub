-- Fix Proofreading Duplicate Assignments
-- Change the LEFT JOIN to a LEFT JOIN LATERAL to ensure only the most recent practice result is bound to the assignment record.
-- Created: 2026-04-21 14:40:00
...
AS $$
  ...
  LEFT JOIN LATERAL (
    SELECT id, accuracy_percentage
    FROM proofreading_practice_results
    WHERE assignment_id = pa.id

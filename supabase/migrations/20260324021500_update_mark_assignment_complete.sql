...
-- Path: supabase/migrations/20260324021500_update_mark_assignment_complete.sql
...
BEGIN
  ...
  ELSIF p_assignment_type = 'reading' THEN
    ...
    INTO v_practice_id, v_title, v_reward_coins
    FROM reading_practice_assignments a -- Assuming table exists based on grep results
  ...
  -- Using dynamic SQL if necessary, but fixed blocks are safer here
  ...
  ELSIF v_table_name = 'reading_practice_assignments' THEN
    -- Fallback for reading if table name is different
    EXECUTE format('UPDATE %I SET completed = true, completed_at = now() WHERE id = %L AND user_id = %L AND (completed = false OR completed IS NULL)', 
                   v_table_name, p_assignment_id, v_user_id);
  END IF;

  -- 3. Return success if update was successful
  IF FOUND THEN
    RETURN TRUE;
  END IF;

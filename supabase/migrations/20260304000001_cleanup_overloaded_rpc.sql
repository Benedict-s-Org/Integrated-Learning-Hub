-- Resolve function overloading ambiguity by dropping older versions
-- Postgres considers functions with different parameter sets as different entities.

-- 1. Drop the ambiguous versions (if they exist)
DROP FUNCTION IF EXISTS public.increment_room_coins(uuid, integer, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.increment_room_coins(uuid, integer, text, uuid, uuid, boolean);

-- 2. Recreate the single authoritative version

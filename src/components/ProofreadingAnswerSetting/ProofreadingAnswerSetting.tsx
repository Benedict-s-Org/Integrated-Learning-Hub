...
const ProofreadingAnswerSetting: React.FC<ProofreadingAnswerSettingProps> = ({
  ...
  const [tips, setTips] = useState<Map<number, string>>(new Map());
  ...
  const handleTipChange = (lineNumber: number, value: string) => {
    setTips(prev => {
      const newMap = new Map(prev);
      newMap.set(lineNumber, value);
      return newMap;
  ...
  const handleAIAutoDetect = async () => {
    ...
    try {
      ...
      const response = await fetch(`${supabaseUrl}/functions/v1/ai-proofread`, {
        method: 'POST',
        headers: {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="border border-gray-300 px-4 py-3">
                          <input
                            type="text"
                            value={corrections.get(sentence.lineNumber) || ''}
                        ...
                        <td className="border border-gray-300 px-4 py-3">
                          <input
                            type="text"
                            value={tips.get(sentence.lineNumber) || ''}
                            onChange={(e) => handleTipChange(sentence.lineNumber, e.target.value)}
                            placeholder="Optional hint"
                            className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent italic"
                            data-source-tsx="ProofreadingAnswerSetting Tip Input|src/components/ProofreadingAnswerSetting/ProofreadingAnswerSetting.tsx"
                          />
                        </td>

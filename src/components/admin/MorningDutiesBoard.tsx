...
export const MorningDutiesBoard: React.FC<MorningDutiesBoardProps> = ({ 
    ...
    const processExemption = async (isExempted: boolean, reason?: string) => {
        try {
            let targetLogId = logs[selectedStudent!.id]?.id;

            if (!targetLogId && isExempted) {
                const today = format(new Date(), 'yyyy-MM-dd');
                const { error: upsertError } = await supabase.rpc('upsert_morning_duty_log', {
                    p_log_date: today,
                    p_student_id: selectedStudent!.id,
                    p_status: 'todo',
                    p_set_by: 'teacher',
                ...
                const { data: newLog, error: fetchError } = await supabase
                    .from('morning_duty_logs')
                    .select('id')
            ...
            if (!targetLogId) throw new Error("Could not determine log ID");

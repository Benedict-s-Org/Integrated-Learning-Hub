...
export function DevPanel({ tileSize, onTileSizeChange, showGrid, onShowGridChange, gridMode, onGridModeChange }: DevPanelProps) {
  ...
  return (
    <div
      className={`fixed right-0 top-0 h-full flex flex-col border-l border-slate-200 bg-white/95 backdrop-blur-sm shadow-xl transition-all duration-300 z-40 ${isOpen ? 'w-80' : 'w-0'
        }`}
    >
      {/* Toggle Button */}
      <button
